...
router.post("/transfer", async function (request, response) {
  ...
  if(weiBalance < txFee) {
    return response.status(400).json({
      msg: `Insufficient balance. transaction balance is ${weiBalance}.`,
    });
  }
  console.log(9);
...
router.get("/track/:wallet_address/:contract_address", async function (
  ...
  try {
    let tx = await axios.get(
      `${getEtherscanApiUrl(config.daemons.polygon.chainId)}/api?module=account&action=tokentx&contractaddress=${req.params.contract_address}&address=${req.params.wallet_address}&sort=asc&apikey=R3NZBT5BV4WK3VER42TJ3B5UK4WYEDZENH`
    );
