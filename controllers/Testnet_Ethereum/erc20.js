...
var web3 = new Web3(new Web3.providers.HttpProvider(config.daemons.test_eth.provider));
...
router.post("/transfer", async function (request, response) {
  ...
  let transactionObject = {
    ...
    chainId: config.daemons.test_eth.provider.chainId,
  };
...
router.get("/fetchtxreceipt/:hash", async function (req, response) {
  var finalResponse = null;
  try {
    if (req.params) {
      if (!req.params.hash) {
        ...
        if (hash.length == 66) {
          ResponseCode = 200;
          finalResponse = await getTransactionReceipt(hash);
          ResponseMessage = "Completed";
        } else {
          ResponseMessage = "Invalid Hash";
          ResponseCode = 400;
...
router.get("/track/:wallet_address/:contract_address", async function (
  ...
  try {
    let tx = await axios.get(
      `${getEtherscanApiUrl(config.daemons.test_eth.chainId)}/api?module=account&action=tokentx&contractaddress=${req.params.contract_address}&address=${req.params.wallet_address}&sort=asc&apikey=R3NZBT5BV4WK3VER42TJ3B5UK4WYEDZENH`
    );
